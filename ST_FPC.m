%% ---Stretching Folding Prection Correction---
...
% Prediction offset(Gauss) --> pre_dvv --> corrected offset(Loop) -->c_dvv
% offset cannot be directly considered as clock error

%-------------------
warning('off'); 
% python warning for map_coordinates   
% map_coordinates contains a extra filter  
% the filter's code seems under cover    
clear;

%%------Setup-------

path = '/media/baimanight/red/right_lag/Test_1~2s';

% Like, baimanight/1~2s/STACKS
% path='baimanight/1~2s' is recommended

directory = '/Str';
filter = '/01'; 
stack = '/001_DAYS';
REF = strcat(path,'/STACKS/01/REF');
pathi = strcat(path,'/STACKS',filter,stack);
...
crep1(mid+minlag*sample_rate:mid+maxlag*sample_rate) = 1; crepl = crep1;
crep2(mid-maxlag*sample_rate:mid-minlag*sample_rate) = 1; crepr = crep2;
...
%% ----loop for corrected offset----
   ...
   off_right = foldlr(rightamp',crepl,crepr,corre_offse);               
   corre_coeff(coffset_loop)=corr(cREF_str(max_index,:)',off_right');       
   end
...
%-----move Fold average-----
    ...
    crepr = [crepl(1-offset:end) zeros(1,-offset)];
    end  
...
function    [offset] =  eliminate(offset,maxdiff,dura_lag)
 ...
 %  cut oscillating offset (fake clock error)
 %  Gauss maybe also work and is more easier  --> eliminate the edge
 %  a new simple function is advised 

 % obviously, there are some abnormal situations 
 % I just ignore some of these   
...
% Unfortunately, below section is not universal, so above code is introduced
% for mixed oscillating zone and short 'fake' normal zone    
